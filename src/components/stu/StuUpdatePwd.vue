  ...
  <div class="updatepwd">
    <el-form ref="form" :model="password" label-width="480px">
      <el-form-item style="width:55%">
        <p class="username">学生用户 {{ this.sessionusername }}</p>
      </el-form-item>
...
export default {
  ...
  created() {
    this.sessionusername = sessionStorage.getItem('stuUsername')
    this.sessionpwd = sessionStorage.getItem('stuPassword')
      ...
      if (this.sessionpwd == this.password.oldpwd) {
        this.$confirm('确认修改？')
          .then((_) => {
            ...
            axios
              .post('http://localhost:8090/stu/updatePwdById', json)
              .then((res) => {})
              .catch((err) => {})
