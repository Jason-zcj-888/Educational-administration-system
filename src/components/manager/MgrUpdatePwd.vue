  ...
  <div class="updatepwd">
    <el-form ref="form" :model="password" label-width="480px">
      <el-form-item style="width:55%">
        <p class="username">管理员用户 {{ this.sessionusername }}</p>
      </el-form-item>
      ...
      <el-form-item style="width:55%" class="updateConfirm">
...
export default {
  ...
  created() {
    this.sessionusername = sessionStorage.getItem('managerUsername')
    this.sessionpwd = sessionStorage.getItem('managerPassword')
      ...
      if (this.sessionpwd == this.password.oldpwd) {
        this.$confirm('确认修改？')
          .then((_) => {
            ...
            axios
              .post('http://localhost:8090/manager/updatePwdById', json)
              .then((res) => {})
              .catch((err) => {})
